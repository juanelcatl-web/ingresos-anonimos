    ...
    <header className="sticky top-0 z-50 bg-dark-bg/80 backdrop-blur-xl border-b border-dark-border">
      <div className="max-w-2xl mx-auto px-4 h-14 flex items-center justify-between">
        <button onClick={() => onNavigate('dashboard')} className="flex items-center gap-2">
          ...
          <span className="font-extrabold text-white text-sm tracking-tight hidden sm:block">Income<span className="text-brand-green">Anon</span></span>
        </button>
        <nav className="hidden sm:flex items-center gap-1">
          {nav.map(item => (
            <button key={item.id} onClick={() => onNavigate(item.id)}
              className={`px-3 py-1.5 rounded-lg text-xs font-semibold transition-all ${activePage === item.id ? 'bg-brand-green/15 text-brand-green' : 'text-white/50 hover:text-white hover:bg-white/5'}`}>
              {item.label}
            </button>
          ))}
        </nav>
